...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):        
        ...
        if CustomUser.objects.filter(username__iexact=user_name).exists() and CustomUser.objects.filter(email__iexact=email).exists():
...
class Fast2SMSSerializer(serializers.Serializer):
