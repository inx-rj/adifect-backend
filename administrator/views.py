        ...
        if serializer.is_valid():
            if self.queryset.filter(Q(job=data['job']) & Q(user=data['user']) & Q(job__is_active=False)).exists():
                ...
                for i in agency.skills.all():
                    skills += f'<div style="margin:0px 10px 10px 0px;width:20%;float:left;"><button style="background-color: rgba(36,114,252,0.08);border-radius: ' \
                              f'30px;font-style: normal;font-weight: 600;font-size: 13px;line-height: 18px;text-align: ' \
